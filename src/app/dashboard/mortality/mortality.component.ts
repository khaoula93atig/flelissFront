...
// Disable tooltips
  ...
  getCV(house) {
    ...
    const datepipe: DatePipe = new DatePipe('en-Fr');
    let formatedDate = datepipe.transform(date1, 'yyyy-MM-dd');
    this.dashboardService.getcvByHouseDaily(formatedDate, house).subscribe(data => {
      this.cv = data;
      console.log('cv', data);
    });
  ...
  getWaterTotal(house) {
    ...
    const datepipe: DatePipe = new DatePipe('en-Fr');
    let formatedDate = datepipe.transform(date1, 'yyyy-MM-dd');
    this.dashboardService.getwaterconsumptionTotalByHouse(formatedDate, house).subscribe(data => {
  ...
  getageofflock(event) {
    ...
    const datepipe: DatePipe = new DatePipe('en-Fr');
    let formatedDate = datepipe.transform(date1, 'yyyy-MM-dd');
    this.dashboardService.getAgeFlock(event, formatedDate).subscribe(res => this.ageofflock = res);
  }
  ...
  getweeklyweightbyBreed() {
    ...
    this.standard = [];
    this.exists = [];
    this.dashboardService.getweeklyweightByBreed(this.breed, this.flockId, this.farmId).subscribe(data => {
      console.log(data);
